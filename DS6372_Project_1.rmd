...
msrp_trim %>%
    group_by(Fuel_Type) %>%
    summarise(count = n())
### Conerns about limited Natrual Gas and Electric Vehicle Fuel Types
...
# Make
msrp_trim %>% ggplot(aes(x = Make, y = log_adj_msrp)) +
    geom_boxplot()
msrp_by_make <- msrp_trim %>%
    group_by(Make) %>%
    summarise(Mean = mean(log_adj_msrp))
msrp_by_make <-
    msrp_by_make %>%
    mutate(
        make_group =
...
msrp_trim2$make_group <- as.factor(msrp_trim2$make_group)

msrp_trim2 %>% ggplot(aes(x = make_group, y = log_adj_msrp)) +
    geom_boxplot()
...
Suggest removing electric Fuel Type from the model. The primary correlation from a continuous variable standpoint and a major one from the categorical side having missing data for electric vehicles.

```{r}
msrp_trim3 <- msrp_trim2 %>% filter(Fuel_Type != "Electric")
```
